...
from visualizations import (
    ...
    create_pie_chart, 
    create_age_risk_chart, 
    create_confidence_histogram, 
    create_bmi_bp_scatter
)
...
try:
    # If your load_model() accepts a filename or parameter, you can pass it here.
    # Otherwise, we map it based on selection:
    if model_choice == "Random Forest Classifier":
        active_model = load_model('random_forest_model.pkl')
    else:
        active_model = load_model('logistic_regression_model.pkl')
        
    model_loaded = True
    st.sidebar.success(f"✅ Loaded: {model_choice}")
except Exception as e:
    # Fallback to default load_model() if your utils doesn't take parameters yet
    try:
        active_model = load_model()
        model_loaded = True
        # st.sidebar.warning("⚠️ Loaded default fallback model configuration.")
    except Exception as inner_e:
        st.sidebar.error(f"Error loading model files. Please ensure the .pkl files exist in your directory.")
        model_loaded = False
...
# ================================================
with tab1:
    st.header(f"Patient Data Input — {model_choice}")
    ...
    if predict_clicked and model_loaded:
        ...
        # 2. Make Prediction using model_utils with the active selected model
        prediction, probabilities = predict_risk(active_model, input_data)
        ...
        with res_col1:
            ...
            <div style="background-color:{color};padding:20px;border-radius:10px;text-align:center;">
                ...
                <h4 style="color:white;margin-top:5px;margin-bottom:5px;font-weight:300;">({model_choice})</h4>
        ...
        with insight_col1:
            ...
            try:
                fig_importance = create_feature_importance_chart(active_model, feature_columns)
                st.plotly_chart(fig_importance, use_container_width=True)
            except Exception as e:
                st.info("Global Feature Importance chart is not supported natively by this model configuration.")
        ...
        with insight_col2:
            ...
            try:
                fig_shap = generate_shap_waterfall(active_model, input_data, prediction)
                st.pyplot(fig_shap)
            except Exception as e:
                st.info("SHAP Analysis is currently unavailable for this specific configuration model.")

# ================================================
# TAB 2: BATCH UPLOAD (CSV)
# ================================================
with tab2:
    st.header(f"Batch Prediction via CSV — {model_choice}")
    ...
    if uploaded_file is not None and model_loaded:
        ...
        if all(col in df_batch.columns for col in feature_columns):
            ...
            predictions = active_model.predict(features)
            probabilities = active_model.predict_proba(features)
            ...
            st.download_button(
                label=f"📥 Download {model_choice} Predictions as CSV",
                data=csv_export,
                file_name=f'batch_predictions_{model_choice.lower().replace(" ", "_")}.csv',
                mime='text/csv',
                type="primary"
            )
        else:
